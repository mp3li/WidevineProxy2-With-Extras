...
export class SettingsManager {
    ...
    static async saveMMEConfig(config) {
        await AsyncLocalStorage.setStorage({ mme_config: config });
    }

    static async getMMEConfig() {
        const result = await AsyncLocalStorage.getStorage(["mme_config"]);
        return {
            enabled: false,
            detailLink: "",
            projectFolder: "",
    ...
    static async saveSelectedMetadataGetter(type) {
        await AsyncLocalStorage.setStorage({ metadata_getter_type: type });
    }

    static async getSelectedMetadataGetter() {
        const result = await AsyncLocalStorage.getStorage(["metadata_getter_type"]);
        // Preserve the previously available LPMAEG handoff for existing users.
        return result.metadata_getter_type === "mme" ? "mme" : "lpmaeg";
    }
